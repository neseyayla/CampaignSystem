...
/**
 * Attaches the token to every call to our own API, and signs the customer out the moment the
 * API stops accepting it.
 *
...
export const authInterceptor: HttpInterceptorFn = (request, next) => {
  ...
  return next(authorised).pipe(
    catchError((error: HttpErrorResponse) => {
      // Not on the sign-in call itself: a wrong password is a message to read, not a reason
      // to throw the customer back to a screen they are already on.
